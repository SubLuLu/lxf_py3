...
```py
>>>s = b'\x42\x4d\x38\x8c\x0a\x00\x00\x00\x00\x00\x36\x00\x00
\x00\x28\x00\x00\x00\x80\x02\x00\x00\x68\x01\x00\x00
\x01\x00\x18\x00'
```
...
def bmp_info(data):
    if len(data)<30:
        result='no bmp, data size too small'
    else:
        msg=struct.unpack('<ccIIIIIIHH',data[:30])
        if msg[:2]==(b'B', b'M') or msg[:2]==(b'B', b'A'):
            result={
                'width': msg[6],
                'height': msg[7],
                'color': msg[9]
...
[check\_bmp.py](https://github.com/michaelliao/learn-python3/blob/master/samples/commonlib/check_bmp.py)