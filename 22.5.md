...
1. [Mako](http://www.makotemplates.org/)：用`<% ... %>`和`${xxx}`的一个模板；
2. [Cheetah](http://www.cheetahtemplate.org/)：也是用`<% ... %>`和`${xxx}`的一个模板；
3. [Django](https://www.djangoproject.com/)：Django是一站式框架，内置一个用`{% ... %}`和`{{ xxx }}`的模板。
